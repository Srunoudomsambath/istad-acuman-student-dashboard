    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-2">
        {studentProjectAchievements.map((achievement) => (
          <Card key={achievement.uuid} className="overflow-hidden border-border/60 bg-card/80 shadow-sm transition-all duration-300 hover:-translate-y-1 hover:shadow-xl">
            <CardContent className="space-y-5 p-5">
              <div className={`relative aspect-[16/10] overflow-hidden rounded-3xl border border-border/60 bg-gradient-to-br ${toneForProgram(achievement.program)}`}>
                ...
                <div className="absolute left-4 top-4 flex flex-wrap gap-2">
                  <Badge variant="secondary" className="rounded-full px-3 py-1 text-[11px] font-semibold">{achievement.program}</Badge>
                  <Badge variant="outline" className="rounded-full bg-background/70 px-3 py-1 text-[11px] font-semibold">{achievement.achievementType}</Badge>
                </div>
                <div className="absolute inset-0 flex flex-col items-center justify-center px-6 text-center">
                  <div className="mt-4 flex size-20 items-center justify-center rounded-3xl border border-white/40 bg-white/30 text-2xl font-bold text-foreground shadow-sm backdrop-blur-sm">{achievement.icon}</div>
                  <p className="mt-4 text-2xl font-semibold tracking-tight text-foreground">{achievement.title}</p>
                </div>
              </div>
              <div className="space-y-3">
                <div className="flex flex-wrap items-start justify-between gap-3">
                  ...
                  <Badge variant="secondary" className="rounded-full px-3 py-1">{achievement.tag}</Badge>
                </div>
                <p className="text-sm leading-6 text-muted-foreground">{achievement.description}</p>
                <div className="flex flex-wrap gap-2">
                  {achievement.highlights.map((item) => (
                    <span key={item} className="rounded-full border border-border/60 bg-background/70 px-3 py-1 text-[11px] font-medium text-foreground">{item}</span>
                  ))}
                </div>
                <div className="flex flex-wrap items-center gap-2 pt-1">
                  <Button asChild size="sm" className="gap-2"><Link href={achievement.link}><ExternalLink className="size-4" />View project</Link></Button>
                  <Button asChild variant="outline" size="sm" className="gap-2"><Link href={achievement.video}><PlayCircle className="size-4" />Watch video</Link></Button>
                </div>
              </div>
            </CardContent>
          </Card>
